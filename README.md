...
> **Nota de Seguridad:** El microservicio implementa un filtro de seguridad personalizado (`JwtUniversalAuthFilter`) que valida tokens JWT de forma unificada. Todas las rutas de gestión y el consumo del catálogo de productos (`/api/productos`) requieren obligatoriamente que el usuario haya iniciado sesión (mediante Google OAuth2 o credenciales de Administrador), enviando el token validado a través del API Gateway en el encabezado `Authorization: Bearer <token>`.